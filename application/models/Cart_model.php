...
class Cart_model extends CI_Model
{
  ...
  function get_allApi($id = null)
  {
   ...
   // BACKEND //
    ...
    {
        $this->db->select('id_trans, id_invoice, users.name, transaksi.created_date, transaksi.grand_total, transaksi.status,transaksi.tipe_trx');
        ...
        $this->db->select("
            CASE 
                WHEN transaksi.status = 1 THEN 'Belum Lunas'
                WHEN transaksi.status = 2 THEN 'Lunas'
                WHEN transaksi.status = 3 THEN 'Kedaluwarsa'
        ...
        $this->db->where('transaksi.status !=', 0); // Menggunakan operator tidak sama dengan
        $this->db->where('(transaksi.tipe_trx != 1 OR (transaksi.tipe_trx = 1 AND transaksi.status = 2))', NULL, FALSE); // Kondisi bersarang
        // Di atas, kita mengecek apakah tipe_trx bukan 1, atau jika tipe_trx adalah 1, maka status harus "Lunas"
    
        if ($id != null) {
            $this->db->where('synchronization_plat.id_merchat_upos', $id);
        }
    
        $this->db->order_by('transaksi.id_trans', 'desc'); // Urutkan berdasarkan id_trans secara descending
        return $this->db->get($this->table)->result();
    }
    
      // BACKEND //
      function get_allApiBySycInv($inv= null)
      {
          $this->db->select('id_trans, id_invoice, users.name, transaksi.created_date, transaksi.grand_total, transaksi.status');
          ...
          $this->db->where('transaksi.status !=', 0); // Menggunakan operator tidak sama dengan
          if ($inv != null) {
            $this->db->where('transaksi.id_invoice',$inv);
          }
  ...
  function get_cart_per_customer_finished($id)
  {
    ...
    $this->db->select('
    lapangan.id_lapangan, lapangan.nama_lapangan,lapangan.foto,
    transaksi.id_trans,transaksi.payment_type,transaksi.company_code,transaksi.bank_transfer,transaksi.va, transaksi.id_invoice, transaksi.user_id, transaksi.subtotal,transaksi.diskonPersen, transaksi.diskon, transaksi.grand_total, transaksi.deadline, transaksi.status, transaksi.catatan,
    transaksi_detail.trans_id, transaksi_detail.lapangan_id, transaksi_detail.tanggal, transaksi_detail.jam_mulai, transaksi_detail.durasi, transaksi_detail.jam_selesai, transaksi_detail.harga_jual, transaksi_detail.total,
   ...
   function get_cart_per_customer_finishedCall($id)
  {
