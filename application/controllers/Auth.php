...
class Auth extends CI_Controller {
	...
	public function membership()
	{
		...
		$this->data['cekRequest']  = $this->Request_model->get_by_id($usrid);
		// var_dump($this->data['cekRequest']);
		$this->data['title'] 			= 'Pengajuan Membership';

		$this->data['profil'] = $this->Ion_auth_model->profil();

		$this->load->view('front/auth/membership-form', $this->data);
	}
	
	public function submitFormPengajuan() {
			$usrid=$this->session->userdata('user_id');
			...
			if (!$this->upload->do_upload('identity_card')) {
				...
				$this->db->insert('member_premium_request', $dataPost);

				var_dump($dataPost);
				redirect(site_url('auth/membership'));
			}
	}
